...
const ScoringSystem = () => {
  ...
  useEffect(() => {
    ...
    const correctGroup = jsonData.answerGroups.find(group => group.title === "Automatisch gescoorde antwoorden");
    if (correctGroup) {
  ...
  const getFilteredAnswers = (group) => {
    ...
    return group.answers.filter(answer => !safeHasOwnProperty(scores, answer.id) || animatingItems[answer.id]);
  };
                    ...
                    {filteredAnswers.map((answer) => (
                      ...
                      >
                        <div className="flex-grow">
                          <p className={`text-gray-800 ${safeHasOwnProperty(scores, answer.id) && !hideScored ? 'text-gray-500' : ''}`}>{highlightWaterWord(answer.text)}</p>
                        </div>
